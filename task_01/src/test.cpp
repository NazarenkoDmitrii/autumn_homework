...
fs::path FindTestsDir() {
  {
    ...
    fs::path by_file = p.parent_path().parent_path() / "tests";
    if (fs::exists(by_file) && fs::is_directory(by_file)) return by_file;
  }
...
TEST(Task01FileCases, All) {
  ...
  for (const auto& in_path : in_files) {
    ...
    ASSERT_FALSE(input.empty()) << in_path.string();

    const std::string expected = ReadFileStrict(out_path);
    ASSERT_FALSE(expected.empty()) << out_path.string();
